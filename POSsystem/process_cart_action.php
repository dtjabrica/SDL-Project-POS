...
function processSale() {
    ...
    if (isset($_SESSION['cart'])) {
        ...
        foreach ($_SESSION['cart'] as $item) {
            ...
            $sugarLevelName = $item['sugar_level_name']; // Retrieve sugar level name
            ...
            $stmt = $conn->prepare("INSERT INTO order_items (order_id, order_date, product_name, product_quantity, addon_name, addon_price, addon_quantity, total_item_price, cup_size, cup_size_price, sugar_level_name) VALUES (?, CURRENT_TIMESTAMP, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("isssdissds", $orderId, $productName, $productQuantity, $addonName, $addonPrice, $addonQuantity, $totalItemPrice, $cupSize, $cupSizePrice, $sugarLevelName);
            $stmt->execute();   
        }    
