...
<body>
    ...
    <script>
        ...
        function updateAddonOptions(productSelect) {
            ...
            if (selectedOption.value === '0') { // Check if no product selected
                addonSelect.disabled = true; // Disable addon select
                addonQuantityInput.disabled = true; // Disable addon quantity input
            }
        }
    </script>
    ...
    <form id="addToCartForm" action="add_to_cart.php" method="POST">
        <select name="product_id" onchange="updateAddonOptions(this); updateCupSizeOptions(this); updateSugarLevelOptions(this);">
        ...
        <h2>Sugar Level</h2>
        <select name="sugar_level_id" id="sugarLevelSelect" disabled>
            <option value="">Select Sugar Level</option>
            <?php
            $sugar_levels_result = $conn->query("SELECT * FROM sugar_level");
            while ($sugar_level_row = $sugar_levels_result->fetch_assoc()) {
                echo "<option value='{$sugar_level_row['level_id']}'>{$sugar_level_row['level_name']}</option>";
            }
            ?>
        </select>
    ...
    // Function to update sugar level options based on selected product
    function updateSugarLevelOptions(productSelect) {
        var selectedOption = productSelect.options[productSelect.selectedIndex];
        var sugarLevelSelect = document.getElementById('sugarLevelSelect');

        // Enable sugar level select
        sugarLevelSelect.disabled = false;

        if (selectedOption.value === '0') {
            sugarLevelSelect.disabled = true;
        }
    }
    </script>
    ...
    <form action="process_cart_action.php" method="POST">
        <ul>
            ...
            if (isset($_SESSION['cart'])) {
                ...
                // Check if sugar level is set
                if (isset($item['sugar_level_name'])) {
                    echo "Sugar Level: " . $item['sugar_level_name'] . "<br>";
                }
